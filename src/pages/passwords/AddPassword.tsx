...
export default function AddPassword(props: any) {
  ...
  const calcPwdEntropy = (value) => {
    getPasswordEntropy({ password: value }).then((entropy) => {
      setState({
        pwdEntropy: entropy ? parseFloat(entropy as string).toFixed(2) : 0.0,
      });
    });
  };
  ...
  return (
    ...
    >
      <Form layout="vertical" requiredSymbol={{ position: 'end' }} form={form}>
        ...
        <FormItem label="Password" field="password" required rules={[{ required: true }]}>
          <Input
            ...
            extra={{
              ...
              bottom: [
                <div key="intensity" className="mt-[6px] mb-[0px]">
                  <span className="text-sm font-light">
                    <span>
                      {state.pwdEntropy
                        ? getPasswordLevel(parseFloat(state.pwdEntropy))
                        : 'Quality'}
                    </span>
                    {state.pwdEntropy ? ` (entropy: ${state.pwdEntropy} bits) ` : ''}
                  </span>
                  <div
                    className={classNames(
                      'h-[2px] bg-gradient-to-r ',
                      getEntropyColor(parseFloat(state.pwdEntropy))
                    )}
                  ></div>
                </div>,
              ],

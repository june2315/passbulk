...
import { get, set, setWith, has, omit, cloneDeep } from 'lodash-es';
import { isArray, isObject, isString } from '../../common/utils';
import { iterativelyGetKeys } from './utils';
...
> {
    ...
    public registerWatcher = (item) => {
        ...
        return () => {
            this.registerWatchers = this.registerWatchers.filter(
                (x) => x !== item
            );
        };
    };
        ...
        if (hasField) {
            return this.registerFields.filter(
                (control) =>
                    ...
                    (options?.containFormList || !control.props?.isFormList)
            );
        }
    ...
    private notify = (type: NotifyType, info: StoreChangeInfo<FieldKey>) => {
        if (
            type === 'setFieldValue' ||
            (type === 'innerSetValue' && !info.ignore)
        ) {
    ...
    public getFieldsError = (fields?: FieldKey[]) => {
        ...
        if (isArray(fields)) {
            ...
            this.getRegisteredFields(true, { containFormList: true }).forEach(
                (item) => {
                    if (item.getErrors()) {
    ...
    public resetFields = (fieldKeys?: FieldKey | FieldKey[]) => {
        const prev = cloneDeep(this.store);
        const fields = isString(fieldKeys)
            ? [fieldKeys as FieldKey]
            : fieldKeys;
        if (fields && isArray(fields)) {
            ...
            fields.forEach((field) => {
                set(
                    this.store,
                    field,
                    this.initialValues[field as unknown as keyof FormData]
                );
    ...
    public validate: FormValidateFn<FormData, FieldValue, FieldKey> =
        promisify<FormData>(
            (
                fieldsOrCallback?:
                    | FieldKey[]
                ...
                if (isArray(fieldsOrCallback) && fieldsOrCallback.length > 0) {
                    controlItems = controlItems.filter(
                        (x) => fieldsOrCallback.indexOf(x.props.field) > -1
                    );
                ...
                Promise.all(promises).then((result) => {
                    let errors = {} as ValidateFieldsErrors<
                        FieldValue,
                        FieldKey
                    >;
    ...
    public getFieldsState = (
        fields?: FieldKey[]
    ): { [key in FieldKey]?: FieldState<FieldValue> } => {

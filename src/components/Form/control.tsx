...
import { isEqualWith, set, get } from 'lodash-es';
...
> extends Component<FormControlProps<FormData, FieldValue, FieldKey>> {
    ...
    private triggerStateCollect = () => {
        const { innerCollectFormState } =
            this.context.store.getInnerMethods(true);
        innerCollectFormState();
    };

    // 切换校验状态
    private toggleValidateStatus = (
        status: FormItemProps['validateStatus'] | undefined
    ) => {
    ...
    private clearFormItemError = (field = this.props.field) => {
        ...
        updateFormItem &&
            updateFormItem(field as string, { errors: null, warnings: null });
    };
    ...
    private getFieldValue = () => {
        ...
        return field
            ? store.getInnerMethods(true).innerGetFieldValue(field)
            : undefined;
    };

    public onStoreChange = (
        type: NotifyType,
        info: StoreChangeInfo<FieldKey> & { current: any }
    ) => {
        ...
        const shouldUpdateItem = (extra?: {
            ...
            if (dependencies && shouldUpdate) {
                // warn(
                //     true,
                //     '`shouldUpdate` of the `Form.Item` will be ignored.'
                // );
            }
            if (dependencies) {
                if (
                    isArray(dependencies) ||
                    (dependencies as string[]).some((depField) =>
                        isFieldMatch(depField, fields)
                    )
                ) {
                    if (this.isTouched()) {
                        this.validateField();
            ...
            case 'setFieldValue':
                if (isFieldMatch(field, fields)) {
                    ...
                    if (info.data && 'warnings' in info.data) {
                        this.setWarnings(
                            isNullOrUndefined(info.data.warnings)
                                ? []
    ...
    handleTrigger = (_value, ...args) => {
        const children = (this.childrenElement ||
            this.props.children) as React.ReactNode;
        const { store } = this.context;
        const { field, trigger, normalize, getValueFromEvent } = this.props;

        const value = isFunction(getValueFromEvent)
            ? getValueFromEvent(_value, ...args)
            : _value;
        ...
        if (
            ...
            children.props &&
            children.props[trigger as string]
        ) {
    ...
    renderControl(children: React.ReactNode, id) {
        ...
        const disabled =
            'disabled' in this.props ? this.props.disabled : ctxDisabled;

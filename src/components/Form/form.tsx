...
import { isObject, omit } from '../../common/utils';
...
>(
    ...
    const [formInstance] = useForm<FormData, FieldValue, FieldKey>(
        baseProps.form
    );
    const isMount = useRef<boolean>();

    const props = useMergeProps<FormProps>(
        baseProps,
        defaultProps,
        {}
    ...
    formInstance.scrollToField = (
        ...
        let fieldNode = node.querySelector(
            `#${getFormElementId(id, field as string)}`
        );
        if (!fieldNode) {
            // 如果设置了nostyle， fieldNode不存在，尝试直接查询表单控件
            fieldNode = node.querySelector(
                `#${getFormElementId(id, field as string)}${ID_SUFFIX}`
            );
        }
        fieldNode &&
    ...
    innerMethods.innerSetCallbacks({
        onValuesChange: (value, values) => {
            ...
            formProviderCtx.onFormValuesChange &&
                formProviderCtx.onFormValuesChange(props.id, value);
        },
        ...
        onValidateFail: (errors: {
            ...
            if (props.scrollToFirstError) {
                const options = isObject(props.scrollToFirstError)
                    ? props.scrollToFirstError
                    : {};
                formInstance.scrollToField(
                    (Object.keys(errors) as FieldKey[])[0],
                    options
                );
            }
        },
        ...
        onSubmit: (values) => {
            ...
            formProviderCtx.onFormSubmit &&
                formProviderCtx.onFormSubmit(props.id, values);
            return returnValue;
        },
    });
    ...
    const contextProps = {
        ...
        prefixCls,
        validateTrigger,
        validateMessages: merge(
            {}
            // ctx.locale.Form?.validateMessages,
            // validateMessages
        ),

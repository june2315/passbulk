...
export default function Table(props: any) {
  ...
  const getRowKey = (record: any, index?: number) => record.key || record.id || `row_${index}`;
  ...
  const handleChecked = (checked: boolean, record: any) => {
    ...
    if (checked) {
      ...
      selectedRowKeys = state.selectedRowKeys.filter((id: any) => id !== record.id);
    }
    setState({
      selectedRowKeys,
    });
  ...
  const isCheckedAll = () => {
    return state.selectedRowKeys.length === dataSource.length && dataSource.length;
  };

  const isIndeterminate = () => {
    return state.selectedRowKeys.length > 0 && state.selectedRowKeys.length < dataSource.length;
  };
            ...
            {rowSelection ? (
              <th scope="col" className="px-4 py-4 " style={{ width: 30 }}>
                <Checkbox
                  ...
                  onChange={(checked: boolean) => handleCheckAll(checked)}
                />
              </th>
            ) : null}
              ...
              >
                {rowSelection ? (
                  <td onClick={(e: any) => e.stopPropagation()}>
                    <Checkbox
                      checked={checked}
                      onChange={(checked: boolean) => handleChecked(checked, record)}
                    />
                  </td>
                ) : null}
                {columns.map?.((col: any, colIndex: number) => {
                  ...
                  return (
                    <td className="px-4 py-3 break-all" key={cellKey}>
                      {col.render ? col.render?.(text, record, index) : text || null}
                    </td>
                  );
                })}
              </tr>
